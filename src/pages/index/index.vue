  ...
  <view class="box-border py-3">
    <view class="mx-3 box-border rounded-3 bg-white px-4 py-6 text-center dark:bg-[var(--wot-dark-background2)]">
      ...
      <text class="mb-3 block text-left text-30rpx text-gray-600 leading-relaxed dark:text-gray-300">
        ⚡️ 基于 vitesse-uni-app 由 vite & uni-app & uni-helper 驱动的、深度整合 Wot UI 组件库的快速启动模板
      </text>
    ...
    <demo-block title="工具链介绍" transparent>
      <wd-cell-group border custom-class="rounded-2! overflow-hidden">
        ...
        <wd-cell title="🚦 Router 路由管理" is-link @click="openUrl('https://moonofweisheng.github.io/uni-mini-router/')" />
        <wd-cell title="🌐 Alova 网络请求" is-link @click="openUrl('https://alova.js.org/zh-CN/')" />
        <wd-cell title="🎨 Icon 图标" is-link @click="navigateTo('icon')" />
        <wd-cell title="✨ Unocss 原子化" is-link @click="openUrl('https://unocss.net/')" />
        <wd-cell title="🍍 Pinia 持久化" is-link @click="openUrl('https://pinia.vuejs.org/zh/')" />
        <wd-cell title="💬 Fedback 反馈组件" is-link @click="navigateTo('feedback')" />
        <wd-cell title="🔄 CI/CD 持续集成" is-link @click="navigateTo('ci')" />
        <wd-cell title="🦾 uni-ku/root" is-link @click="openUrl('https://uni-ku.js.org/projects/root/introduction')" />
